...
# 1、构建数据库
db = SQLDatabase.from_uri("mysql+pymysql://root:123456@localhost:3306/book")
# 2、构建大模型
model = ChatOpenAI(
    temperature=0.95,
    model="glm-4",
    openai_api_key="b9555c1df61e441435592b83c044a4e2.qgHyKediqm4A078v",
...
if problem:
    ...
    try:
        ...
        with st.chat_message("assistant"):
            st.write("数据库暂无数据回答此问题")
        st.session_state.history.append({"role": "assistant", "content": "数据库暂无数据回答此问题"})
